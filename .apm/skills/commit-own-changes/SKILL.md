...
2. 查看 git 状态，确认有哪些文件是本轮任务修改的（无需关注其他与本轮任务无关的改动）

```powershell
git status
```

3. 获取本轮改动的具体行号，根据行号暂存修改
...
```powershell
npx git-stage-lines <changed_file>:<修改的行号范围>
```

或 `git add <changed_file>` （仅当全部修改均需包含在内时。）
...
- 禁止 `git add -A`。
- 禁止不使用 `npx git-stage-lines diff` 运行 `git add`。
- 行号必须从 `npx git-stage-lines diff` 获取，不要凭记忆推测。

...
计划文档输出到 /docs/features/{feature}/yymmdd-{feature}.plan.md。
...
使用 /spawn-reviewer 检视代码（而不是使用 bugbot 子代理），指定检视报告输出位置为 /docs/features/{feature}/yymmdd-{feature}.review.md。
...
最后，在征得用户 _书面确认_ 后提交本次修改的文件。遵循历史提交格式。

...
本流程设两道闸门：**背景充分门**（Research 出口，置信闸门）与**计划对齐门**（Align 出口，对齐闸门）。置信闸门由你自行满足；对齐闸门默认**现结**——到达时暂停，交用户确认后前进；`/delay-validation` 激活时改为**挂账**——按最佳判断通过，在决策台账记一笔（闸门、决策、理由）后继续。
...
若代码库庞大，可委托子代理探索并总结，保持主上下文整洁。若适用，使用 /spawn-deep-researcher 进行网络调研（指定输出文件: /docs/features/yymmdd-{feature_name}/yymmdd-{topic}.research.md）。
...
**现结**：请用户检查计划。若用户表达了疑虑、困惑、反对，请仔细理解用户的关注点，重新审视你的计划。
用户不是上帝，用户可能正确也可能搞错。保持独立判断，按需进行进一步调研、搜索、咨询 `expert`、cross-check 等。
持续改进计划，直到你的计划与用户期望完全对齐为止。

**挂账**：输出计划后按最佳判断通过对齐门。在决策台账记录关键选型及理由，以及你识别到但未经用户对齐的风险点，供用户审计。

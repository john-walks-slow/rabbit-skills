...
本流程的置信闸门（实施完成度、静态与测试验证、检视准入）由你自行满足；**验证门**是对齐闸门，默认**现结**——到达时暂停，交用户实机验证确认后前进；`/delay-validation` 激活时改为**挂账**——你先行自验（`/e2e` 激活时执行完整端到端自测），在台账记录验证方式与结果，仅用户可验的项目列入遗留验证清单，继续前进。
...
使用 /spawn-reviewer 检视代码，指定检视报告输出位置：
...
**现结**：向用户简述本次实现，请用户按照验证文档完成验证并填写实际结果、状态和备注/证据。**当用户书面确认功能正常后才进入下一步。**

**挂账**：你先行完成可自验的部分（`/e2e` 激活时执行完整端到端自测），在台账记录验证方式与结果；仅用户可验的项目保留在验证文档中作为遗留验证清单，交付时汇总，不阻塞后续步骤。
...
当用户书面要求时，你可以调整工作流。如：并行 spawn 多个 reviewer；commit 前需确认等。

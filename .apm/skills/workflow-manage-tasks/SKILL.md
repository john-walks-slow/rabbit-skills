...
使用 background 模式并行分派 Agent，每一任务使用独立 Agent，除 hard dependency 外尽量并行（对并行安全性默认保持乐观）。

**所有具体任务全部分配给 Agent 执行**，包括但不限于：代码编写、测试、提交。你自身不执行任何具体任务。

Agent 已内置详细工作流约束。**无需设定 verbose 的提示词和工作方式；无需复述工作流。**。过度限定反而限制其发挥。将经过梳理的简短、清晰、完整的意图传递给 Agent 即可。
